...
public class DownloaderActivity extends Activity {
	...
	private DownloaderReceiver downloaderReceiver;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.pause).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				final Intent intent = new Intent(DownloaderActivity.this,MapDownloaderService.class);
				stopService(intent);
			}
		});
		...
		mTextVwError.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (Build.VERSION.SDK_INT >= 24) {
					uriGraph = FileProvider.getUriForFile(DownloaderActivity.this, "com.mtkj.cnpc.photo.fileprovider", file);
				} else {
					uriGraph = Uri.fromFile(file);
				}
				final Intent intent = new Intent(Intent.ACTION_VIEW).setDataAndType(uriGraph, "text/plain");
		...
		((TextView) findViewById(R.id.tv_title_title)).setText("离线地图下载");
	...
	public void initProgressDialog(){
		...
		progressDialog.setMessage("正在下载地图...");
		progressDialog.show();
	}
	...
	private void registReceiver(){
		...
		filter.addAction("RMaps.Download.Maps");
		this.registerReceiver(downloaderReceiver, filter);
	}
...
//		final String name = Ut.FileName2ID(mFileName+".sqlitedb");
		...
		.putExtra("MapName", mLoadToOnlineCache ? mTileSource.ID : "usermap_" + mFileName + ".sqlitedb")
		.putExtra("center", mCenter.toDoubleString())
	...
	protected void onDestroy() {
		this.unregisterReceiver(downloaderReceiver);
		super.onDestroy();
	}
}