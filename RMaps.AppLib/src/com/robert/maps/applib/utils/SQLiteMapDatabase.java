...
public class SQLiteMapDatabase implements ICacheProvider {
	...
	private static final String SQL_CREATE_info = "CREATE TABLE IF NOT EXISTS info (lat double, lon double, maxzoom Int, minzoom Int, params VARCHAR);";
...
//			throw aException;
		...
		for (int i = 0; i < mDatabase.length; i++) {
			if (mDatabase[i] != null)
				if (mDatabase[i].getPath().toLowerCase(Locale.US).endsWith(SQLITEDB)) {
					...
					if (c != null) {
						if (c.moveToFirst()) {
							zooms[0] = c.getInt(c.getColumnIndex("minzoom"));
							zooms[1] = c.getInt(c.getColumnIndex("maxzoom"));
						}
					}
					c.close();
				}
	...
	public void setParams(String mapID, String mapName, int[] coordArr, int[] zoomArr, int minzoom, int maxzoom) {
		final JSONObject json = getParams();
		try {
			json.put(MAPID, mapID);
			json.put(MAPNAME, mapName);
			json.put(ZOOM, minzoom);
			{
		...
		for (int i = 0; i < mDatabase.length; i++) {
			if(mDatabase[i] != null) {
				if(mDatabase[i].getPath().toLowerCase(Locale.US).endsWith(SQLITEDB)) {
					...
					int lat = (coordArr[2] + coordArr[0]) / 2;
					int lon = (coordArr[3] + coordArr[1]) / 2;
					GeoPoint geoPoint = new GeoPoint(lat, lon);
					String sql = "INSERT INTO 'info' (minzoom, maxzoom, lat, lon) VALUES (?, ?, ?, ?);";
					String[] args = new String[]{String.valueOf(minzoom), String.valueOf(maxzoom), String.valueOf(geoPoint.getLatitude()), String.valueOf(geoPoint.getLongitude())};
					mDatabase[i].execSQL(sql, args);
